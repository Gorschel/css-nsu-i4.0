...
500)}else setTimeout(svan,500)};window.URL=window.URL||window.webkitURL||window.mozURL||window.msURL;navigator.getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;e=function(a){h.src=window.URL&&window.URL.createObjectURL(a)||a;localMediaStream=a;h.play();setTimeout(k,1E3)};MediaStreamTrack.getSources(function(sourceInfos) {
    var cameras = sourceInfos.filter(function(sourceInfo) {
        return sourceInfo.kind == "video";
    });
    var rearCameras = sourceInfos.filter(function(sourceInfo) {
        return sourceInfo.facing == "environment";
    });
    var camera;
    if (rearCameras.length > 0) {
        camera = rearCameras[0];
    } else {
        camera = cameras[0];
    }
    ...
    navigator.getUserMedia?navigator.getUserMedia(constraints,e,b):console.log("Native web camera streaming (getUserMedia) not supported in this browser.");
});qrcode.callback=
a}})(jQuery);